...
const VehiclePurchase = (function () {
    const VEHICLE_API = '/business/api/vehicles';
    const ORDER_API = '/business/api/admin/orders';
    ...
    async function loadSystemUser(userId) {
        const res = await apiRequest('GET', '/system/api/system/users/' + userId, null, true);
    ...
    async function renderCheckout() {
        ...
        rootEl.innerHTML =
            ...
            '<p class="hint">提交后将调用真实下单链路（business → system），并写入订单管理列表。</p>' +
